...
func allowOAuth(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if len(redirectUri) == 0 {
		c.Err = model.NewOAuthAppError("allowOAuth", "invalid_request", "Missing or bad redirect_uri")
		return
	}
	...
	if !app.IsValidRedirectURL(redirectUri) {
		...
		c.Err = model.NewOAuthAppError("allowOAuth", "invalid_request", "Supplied redirect_uri did not match registered callback_url")
		return
	}
