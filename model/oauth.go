...
func (a *OAuthApp) IsValid() *AppError {
	...
	if len(a.Id) != 26 {
		return NewOAuthAppError("OAuthApp.IsValid", "Invalid app id", "")
	}

	if a.CreateAt == 0 {
		return NewOAuthAppError("OAuthApp.IsValid", "Create at must be a valid time", "app_id="+a.Id)
	}

	if a.UpdateAt == 0 {
		return NewOAuthAppError("OAuthApp.IsValid", "Update at must be a valid time", "app_id="+a.Id)
	}

	if len(a.CreatorId) != 26 {
		return NewOAuthAppError("OAuthApp.IsValid", "Invalid creator id", "app_id="+a.Id)
	}

	if len(a.ClientSecret) == 0 || len(a.ClientSecret) > 128 {
		return NewOAuthAppError("OAuthApp.IsValid", "Invalid client secret", "app_id="+a.Id)
	}

	if len(a.Name) == 0 || len(a.Name) > 64 {
		return NewOAuthAppError("OAuthApp.IsValid", "Invalid name", "app_id="+a.Id)
	}

	if len(a.CallbackUrls) == 0 || len(fmt.Sprintf("%s", a.CallbackUrls)) > 1024 {
		return NewOAuthAppError("OAuthApp.IsValid", "Invalid callback urls", "app_id="+a.Id)
	}

	if len(a.Homepage) == 0 || len(a.Homepage) > 256 {
		return NewOAuthAppError("OAuthApp.IsValid", "Invalid homepage", "app_id="+a.Id)
	}

	if len(a.Description) > 512 {
		return NewOAuthAppError("OAuthApp.IsValid", "Invalid description", "app_id="+a.Id)
	}
